<template>
  <div class="container blog-wrapper">
    ...
    <form v-on:submit.prevent = "createBlog">
      <p>
        <label for="" class="control-label">ชื่อชมรม : </label>
      ...
      <form enctype="multipart/form-data" novalidate>
        <div class="dropbox">
          <input type="file" multiple :name="uploadFieldName" :disabled="isSaving" @change="filesChange($event.target.name, $event.target.files); fileCount = $event.target.files.length"
            ...
            <p v-if="isInitial">
              ลากไฟล์รูปภาพมาที่นี้ <br> หรือ คลิกเพื่อเลือก
            </p>
            <p v-if="isSaving">
      ...
      <div>
        <transition-group tag="ul" name="fade" class="pictures">        
          <li v-for="picture in pictures" v-bind:key="picture.id">              
            ...
            <br />  
            <button class="btn btn-xs btn-info" v-on:click.prevent="useThumbnail(picture.name)">เลือก</button>
      ...
      <p>
        <label class="control-label">ประเภทชมรม :</label>
        <input type="text" v-model="blog.category" class="form-control">
      </p> 
      <p>
        <button class="btn btn-success" type="submit">สร้าง</button>
        <button class="btn btn-default" type="button" v-on:click="navigateTo('/blogs')">ย้อนกลับ</button>
      </p> 
    </form>   
    <br>    
  </div>
